...
const state = {
  ...
  saved: new Set(JSON.parse(localStorage.getItem("aufp_saved") || "[]")),
  auto: localStorage.getItem("aufp_auto") !== "false",
  compact: localStorage.getItem("aufp_compact") === "true",
  brief: localStorage.getItem("aufp_brief") || "", timer: null, installPrompt: null
};

function esc(v="") { return String(v).replace(/[&<>'"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"}[c])); }
function safeUrl(v="") { try { const u=new URL(v); return /^https?:$/.test(u.protocol)?u.href:"#"; } catch { return "#"; } }
function normalise(a) { return {id:a.id||crypto.randomUUID(),title:a.title||"Untitled story",link:safeUrl(a.link),source:a.source||"Unknown source",publishedAt:a.publishedAt||new Date().toISOString(),category:a.category||"all",official:Boolean(a.official),description:a.description||"Open the original source for the full context."}; }
...
function toast(text) { const n=$("toast"); n.textContent=text; n.classList.add("show"); clearTimeout(toast.t); toast.t=setTimeout(()=>n.classList.remove("show"),2300); }
...
async function loadNews(silent=false) {
  ...
  $("refreshButton").classList.add("loading"); $("syncStatus").textContent="Refreshing sourced headlines…";
  try {
    const r=await fetch(`./news.json?t=${Date.now()}`,{cache:"no-store",headers:{Accept:"application/json"}}); if(!r.ok) throw Error(`HTTP ${r.status}`);
    const data=await r.json(); if(!Array.isArray(data.articles)||!data.articles.length) throw Error("No articles");
    state.articles=data.articles.map(normalise); localStorage.setItem("aufp_cache",JSON.stringify({articles:state.articles,generatedAt:data.generatedAt}));
    $("syncStatus").textContent="Australian feeds connected"; $("lastUpdated").textContent=`Feed updated ${ago(data.generatedAt||new Date().toISOString())}`;
  } catch(err) {
    const cache=JSON.parse(localStorage.getItem("aufp_cache")||"null"); state.articles=(cache?.articles?.length?cache.articles:FALLBACK).map(normalise);
    $("syncStatus").textContent=cache?.articles?.length?"Offline · showing cached feed":"Preview feed"; $("lastUpdated").textContent=cache?.generatedAt?ago(cache.generatedAt):"";
    if(!silent) toast("Live feed unavailable; showing saved headlines"); console.warn(err);
  } finally { $("refreshButton").classList.remove("loading"); render(); }
}

function save(id) { state.saved.has(id)?state.saved.delete(id):state.saved.add(id); localStorage.setItem("aufp_saved",JSON.stringify([...state.saved])); render(); toast(state.saved.has(id)?"Story saved":"Removed from saved"); }
...
function quickBrief() {
  ...
  localStorage.setItem("aufp_brief",state.brief); $("briefText").textContent=state.brief; $("shareBriefButton").disabled=false;
}
...
function bind() {
  ...
  $("clearSavedButton").onclick=()=>{state.saved.clear();localStorage.setItem("aufp_saved","[]");render();toast("Saved stories cleared");};
  ...
  $("autoRefreshToggle").onchange=e=>{state.auto=e.target.checked;localStorage.setItem("aufp_auto",state.auto);timer();};
  $("compactToggle").onchange=e=>{state.compact=e.target.checked;localStorage.setItem("aufp_compact",state.compact);document.body.classList.toggle("compact",state.compact);};
...
function init() {
  ...
  bind();timer();loadNews();if("serviceWorker" in navigator&&location.protocol.startsWith("http"))navigator.serviceWorker.register("./sw.js").catch(console.warn);
}
