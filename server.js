...
const WIDGET_URI = "ui://widget/aurora-v1.html";
...
function calculateScore() {
  ...
  return Math.round(spendScore * .2 + focusScore * .3 + profile.energy * .2 + progressScore * .3);
}
...
function createAuroraServer() {
  ...
  registerAppTool(server, "show_dashboard", {
    ...
    inputSchema:{},
    outputSchema:{
      profile:z.object({spend:z.number(),focusMinutes:z.number(),energy:z.number(),doneCount:z.number(),score:z.number(),brief:z.string()}),
      tasks:z.array(z.object({id:z.string(),title:z.string(),detail:z.string(),tag:z.string(),completed:z.boolean()}))
    },
  ...
  registerAppTool(server, "update_metrics", {
    ...
    profile = { ...profile, ...Object.fromEntries(Object.entries(args).filter(([,v]) => v !== undefined)) };
  ...
  registerAppTool(server, "analyse_day", {
    ...
    if (q.includes("钱") || q.includes("支出") || q.includes("save"))
      brief = "最快的改善方式是检查一项经常性费用，并对非必要购买设置 24 小时延迟。";
    else if (q.includes("累") || q.includes("精力") || q.includes("sleep"))
      brief = "你的精力趋势提示需要降低认知负担。完成一件任务、短暂散步，并提前 45 分钟停止工作。";
    else if (q.includes("工作") || q.includes("专注") || q.includes("focus"))
...
const httpServer = createServer(async (req,res) => {
  ...
  if (req.method === "GET" && url.pathname === "/") {
    ...
    return res.end(JSON.stringify({name:"Aurora ChatGPT App",status:"ok",mcp:"/mcp"}));
  }
